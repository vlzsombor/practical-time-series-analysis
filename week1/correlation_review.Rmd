...
$$ p(X,Y) = E \left[ \left(\frac{X - \mu_x}{\sigma_X}\right) \left(\frac{Y - \mu_Y}{\sigma_Y} \right)\right]$$

To compute the correlation estimate $r$ of data, we compute:

$$ r \equiv \hat{p} \equiv \frac{1}{n-1} \sum \left( \frac{x_i - \hat{x}_i}{s_X} \right) \left( \frac{y_i - \hat{y}_i}{s_Y} \right) $$
