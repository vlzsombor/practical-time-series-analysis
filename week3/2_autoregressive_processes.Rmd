...
(1 + \theta_1 B + \theta_2 B^2 + ...) Z_t
$$

We can then easily compute the **expected value of $X_t$**:

$$
E[X_t] = E[(1 + \theta_1 B + \theta_2 B^2 + ...) Z_t] = \\
E[Z_t + \theta_1 Z_{t-1} + \theta_2 Z_{t-2} + ...] = \\
E[Z_t] + \theta_1 E[Z_{t-1}] + \theta_2 E[Z_{t-2}] + ... = 0
$$
...
$$
V[X_t] = \\
V[(1 + \theta_1 B + \theta_2 B^2 + ...) Z_t] = \\
V[Z_t] + \theta_1² V[Z_{t-1}] + \theta_2² V[Z_{t-2}] + ... = \\
\sigma_z^2 + \theta_1² \sigma_z^2 + \theta_2^2 \sigma_z^2 + ... \\
\sigma_z^2 \sum^{\infty}_{k=0} \theta_i^2
$$
...
$$
\gamma(k) = \sigma^2_Z \sum^{q-k}_{i=0} \theta_i \theta_{i+k}
$$

For an $AR(p)$ process:

$$
\gamma(k) = \sigma^2_Z \sum^{\infty}_{i=0} \theta_i \theta_{i+k}
$$

Note the main difference is that contributions to covariance are limited to positions within $q$ in $MA$ while all positions contribute to covariance in $AR$. Note also that this converges if $\sum^{\infty}_{i=0} | \theta_i |$ absolutely converges.
...
$$
p(k) = \frac{\gamma(k)}{\gamma(0)} = \frac{\sigma^2_Z \sum^{\infty}_{i=0} \theta_i \theta_{i+k}}{\sigma^2_Z \sum^{\infty}_{i=0} \theta_i \theta_{i}} = \frac{\sum^{\infty}_{i=0} \theta_i \theta_{i+k}}{\sum^{\infty}_{i=0} \theta_i \theta_{i}}
$$